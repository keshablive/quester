...
jest.mock('@tanstack/react-query', () => ({
  useQuery: jest.fn((options: any) => {
    // Different mocks based on queryKey
    const queryKey = options?.queryKey?.[0];
    if (queryKey === 'certificates') {
      return {
    ...
    // Default for marketplace and other queries
    return {
      data: { listings: [], totalCount: 0 },
      isLoading: false,
      error: null,
      refetch: jest.fn(),
    };
...
jest.mock('@/lib/api/courses', () => ({
  useCourses: jest.fn(() => ({
    data: [{ id: '1', title: 'Test Course', price: 0, difficulty: 'beginner', published: true }],
    isLoading: false,
    error: null,
    refetch: jest.fn(),
  })),
...
// Mock Social hooks for Feed
jest.mock('@/lib/hooks/useSocial', () => ({
  useSocial: jest.fn(() => ({
    posts: [],
    loading: false,
    error: null,
...
// Mock Marketplace (no direct API hook used)
...
jest.mock('@/lib/hooks/use-gamification-feedback', () => ({
  useGamificationFeedback: jest.fn(() => ({
    awardXP: jest.fn(),
    dismissXPAnimation: jest.fn(),
    dismissLevelUpModal: jest.fn(),
