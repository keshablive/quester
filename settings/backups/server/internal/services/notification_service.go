...
func (s *NotificationService) DeleteNotification(ctx context.Context, notificationID, tenantID, userID string) error {
	...
	return s.notificationRepo.Delete(ctx, notification.ID)
}
...
func (s *NotificationService) TrackNotificationDeliveryStatus(ctx context.Context, notificationID string, channel string, status string, deliveredAt *time.Time, errorMsg string) error {
	// Fetch notification
	notification, err := s.notificationRepo.FindByID(ctx, notificationID)
	if err != nil {
