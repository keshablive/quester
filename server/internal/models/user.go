...
type User struct {
	...
	// Timezone for streak calculations (006-course-gamification)
...
// GetID implements auth.ClaimsProvider interface
// Returns the user's unique identifier for JWT subject claim
func (u *User) GetID() uuid.UUID {
	return u.ID
}

// GetTenantID implements auth.ClaimsProvider interface
// Returns the user's tenant identifier for multi-tenancy JWT claim
func (u *User) GetTenantID() uuid.UUID {
	return u.TenantID
}

// GetRole implements auth.ClaimsProvider interface
// Returns the user's role as a string for JWT role claim
