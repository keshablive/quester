...
func (s *AchievementService) GetAchievement(ctx context.Context, tenantID, achievementID uuid.UUID) (*models.Achievement, error) {
	return s.achievementRepo.FindByID(ctx, tenantID, achievementID)
}
...
func (s *AchievementService) TrackProgress(ctx context.Context, userID uuid.UUID, achievementType models.AchievementType, category models.AchievementCategory, increment int) error {
	...
	achievements, err := s.achievementRepo.FindByCategory(ctx, uuid.Nil, category) // Note: Using uuid.Nil for global achievements; tenant-specific filtering can be added later
	if err != nil {
	...
	for _, achievement := range achievements {
		...
		userAchievement, err := s.achievementRepo.FindOrCreateUserAchievement(ctx, achievement.TenantID, userID, achievement.ID)
		if err != nil {
			s.LogInfo("Error creating user achievement: %v", err)
			continue
		}
		...
		if err := s.achievementRepo.IncrementUserAchievementProgress(ctx, userID, achievement.ID, increment); err != nil {
			s.LogInfo("Error incrementing progress: %v", err)
			continue
		}
		...
		if err != nil {
			s.LogInfo("Error reloading user achievement: %v", err)
			continue
		}
		...
		if userAchievement.CurrentCount >= achievement.TargetCount && !userAchievement.Completed {
			if err := s.completeAchievement(ctx, userID, achievement.ID); err != nil {
				s.LogInfo("Error completing achievement: %v", err)
			}
		}
	}
...
func (s *AchievementService) TrackXPThreshold(ctx context.Context, userID uuid.UUID, currentXP int) error {
	...
	for _, achievement := range achievements {
		...
		if currentXP >= achievement.TargetCount {
			...
			userAchievement, err := s.achievementRepo.FindOrCreateUserAchievement(ctx, user.TenantID, userID, achievement.ID)
			if err != nil {
				s.LogInfo("Error creating user achievement: %v", err)
				continue
			}
	...
	for _, achievement := range achievements {
		...
		if currentStreak >= achievement.TargetCount {
			...
			if err := s.achievementRepo.UpdateUserAchievementProgress(ctx, userAchievement); err != nil {
				s.LogInfo("Error updating progress: %v", err)
				continue
			}
...
func (s *AchievementService) completeAchievement(ctx context.Context, userID, achievementID uuid.UUID) error {
	...
	achievement, err := s.achievementRepo.FindByID(ctx, user.TenantID, achievementID)
	if err != nil {
	...
	if achievement.XPReward > 0 {
		if err := s.awardXP(ctx, userID, achievement.XPReward); err != nil {
			s.LogInfo("Failed to award XP for achievement %s: %v", achievement.Name, err)
		} else {
			s.LogInfo("Achievement completed: user_id=%s, achievement=%s, xp_reward=%d",
				userID, achievement.Name, achievement.XPReward)
		}
	}

	// Award badge if specified (integrated via BadgeService)
	if achievement.BadgeID != uuid.Nil {
		s.LogInfo("Badge award pending: user_id=%s, badge_id=%s", userID, achievement.BadgeID)

		// Create a lightweight mapping record (user_achievement_badges) so we can show a badge earned in the achievements UI
		if err := s.achievementRepo.CreateUserAchievementBadge(ctx, user.TenantID, userID, achievementID, achievement.BadgeID); err != nil {
			s.LogInfo("Failed to persist user achievement badge mapping: %v", err)
		}
		...
		if s.badgeService != nil {
			...
			user, err := s.userRepo.FindByID(ctx, userID)
			if err != nil {
				s.LogInfo("Failed to lookup user for badge award: %v", err)
			} else {
				go func(tID, uID, bID uuid.UUID) {
					if err := s.badgeService.AwardBadge(context.Background(), tID, uID, bID); err != nil {
						s.LogInfo("Failed to auto-award badge: %v", err)
					} else {
						s.LogInfo("Badge awarded successfully: user=%s, badge=%s", uID, bID)
					}
	...
	// Achievement unlock notification (sent via NotificationService below if available)
	s.LogInfo("Achievement unlocked: user_id=%s, achievement=%s", userID, achievement.Name)
	...
	if s.notificationSvc != nil {
		...
		go func() {
			if _, err := s.notificationSvc.CreateNotification(context.Background(), achievement.TenantID.String(), userID.String(), models.NotificationTypeBadgeEarned, title, message, actionURL, iconURL, models.NotificationPriorityHigh, channels, metadata); err != nil {
				s.LogInfo("Failed to send achievement notification: %v", err)
			}
		}()
	}
