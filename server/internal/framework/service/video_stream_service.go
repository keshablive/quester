...
import (
	...
	"github.com/keshablive/quester/internal/adapters"
	"github.com/keshablive/quester/internal/framework/streaming"
...
func NewVideoStreamService(db *gorm.DB, config *StreamingConfig) (*VideoStreamService, error) {
	// Create stream repository adapter for the RTMP server
	streamRepo := adapters.NewStreamRepositoryAdapter(db)
	...
	// Set RTMP callbacks using the adapter
	authHandler := streaming.NewRTMPAuthHandler(streamRepo)
