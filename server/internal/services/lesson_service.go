...
func (s *LessonService) Complete(req *CompleteLessonRequest) (*models.LessonCompletion, error) {
	...
	if err == nil {
		...
		if err := enrollment.CheckAndIssueCertificate(s.db); err != nil {
			// Certificate issuance error logged but doesn't fail completion
			log.Warnf("Certificate issuance check failed: %v", err)
		}
	}
...
// Note: Direct DB update - will integrate with LeaderboardService when available
